...
def command_igoto(entity, argument=""):
    if not entity:
    ...
    if not argument:
        syntax = get_command_syntax(entity, "command_igoto")
        entity.send_output(syntax, break_line=False)
        return False

    target, room, last_message = goto_entity_handler(entity, argument, "items")
    ...
    goto_message(entity, room, "igoto")
    entity = entity.from_location(1, use_repop=True)
    entity.to_location(room, use_look=True)
    entity.act("$n arriva tramite un igoto da %s %s" % (old_coords, old_area_code), TO.ADMINS)
...
def get_syntax_template(entity):
    ...
    syntax = ""
    syntax += "igoto\n"
    syntax += "igoto <codice oggetto o suo prefisso>\n"
    syntax += "igoto <nome oggetto o suo prefisso>\n"
