...
LONG = ["poggia qui a terra",
        "si nasconde qui in basso",
        "si vede appena",
        "si mimetizza in un angolo",
        "è stat$o gettat$o qui"]
...
GEMME_DATA = {
    ...
    "cabochon ellissoidale": [
        "cabollissoidale",
        "Quest$O $ColoredName di forma ellissoidale è levigat$O con tale precisione da non sembrare opera di un mortale. La luce dell'ambiente è catturata tutta e rifratta con tanta efficienza da farl$o brillare come di luce propria.",
...
def before_giving(player, gemma_grezza, tagliapietre, direction, behavioured):
    ...
    if "work_with_code" in tagliapietre.specials and tagliapietre.specials["work_with_code"] != player.code:
        to_say = "a %s ... dammi tempo che son già occupato con %s" % (player.code, tagliapietre.specials["work_with_name"])
    ...
    command_say(tagliapietre, to_say)
    crea_gemma(id_gemma, player, tagliapietre, gemma_grezza, tagliapietre.location)
    return False
#- Fine Funzione -


def crea_gemma(id_gemma, player, tagliapietre, gemma_grezza, room):
    # Ce ne sono due di specials per rendere compatibile lo script nel qual
    # caso si voglia estendere il funzionamento anche ai mob
    tagliapietre.specials["work_with_code"] = player.code
    tagliapietre.specials["work_with_name"] = player.get_name(looker=tagliapietre)
    ...
    gemma_lavorata.long = "$N " + long_descr
    gemma_lavorata.weight = gemma_grezza.weight

    gemma_lavorata.life = gemma_grezza.life
    gemma_lavorata.max_life = gemma_grezza.max_life
#    gemma_lavorata.mana = gemma_grezza.mana
#    gemma_lavorata.max_mana = gemma_grezza.max_mana
#    gemma_lavorata.vigor = gemma_grezza.vigor
...
def after_shake(player, target, argument):
    delete_specials(target)
#- Fine Funzione -


def delete_specials(tagliapietre):
    if "work_with_code" in target.specials:
        del(target.specials["work_with_code"])
    if "work_with_name" in target.specials:
        del(target.specials["work_with_name"])
#- Fine Funzione -
